...
if (!$listing) {
    echo "<p style='text-align:center;padding:40px'>
            <i class="fas fa-exclamation-triangle"></i> Item not available. <a href='/ums/index.php'>Go back</a>
          </p>";
    exit;
}
...
if ($listing['seller_id'] === $_SESSION['user_id']) {
    echo "<p style='text-align:center;padding:40px'>
            <i class="fas fa-exclamation-triangle"></i> You cannot buy your own listing.
            <a href='/ums/index.php'>Go back</a>
          </p>";
    exit;
}
    ...
    if (empty($error)) {
        ...
        if (isset($response['CheckoutRequestID'])) {
            ...
            $pdo->prepare("
                ...
                (order_id, phone, amount, platform_fee, merchant_request_id, checkout_request_id)
                VALUES (?, ?, ?, ?, ?, ?)
            ")->execute([
                $order_id,
                $phone,
...
$fee = calculateFee($listing['price']);
?>
        ...
        <div class="pay-summary">
            ...
            <div class="pay-item">
                <?php if ($listing['image']): ?>
                    <img src="/ums/uploads/items/<?= htmlspecialchars($listing['image']) ?>"
                         alt="item">
                <?php else: ?>
                    <div class="pay-no-img"><i class="fa-solid fa-box"></i></div>
                <?php endif; ?>
                <div>
            ...
            <div class="pay-breakdown">
                ...
                <div class="pay-row fee-row-info">
                    <span>
                        <i class="fa-solid fa-building-columns"></i> Platform Fee (<?= PLATFORM_FEE_PERCENT ?>%)
                        <br><small>Funds student welfare & platform</small>
                    </span>
                    <span>KSh <?= number_format($fee['fee'], 2) ?></span>
                </div>
                <div class="pay-row">
                    <span><i class="fa-solid fa-coins"></i> 💰 Seller Receives</span>
                    <span>KSh <?= number_format($fee['seller_amount'], 2) ?></span>
                </div>
                <div class="pay-row total">
                    <span><i class="fa-solid fa-wallet"></i> You Pay</span>
                    <span>KSh <?= number_format($fee['price'], 2) ?></span>
                </div>
            </div>

            <div class="fee-notice" style="margin-top:12px;">
                <i class="fa-solid fa-info-circle"></i> A <strong><?= PLATFORM_FEE_PERCENT ?>% platform fee</strong>
                is deducted from the seller's payout. You pay the listed price only.
            </div>
        </div>
            ...
            <?php if (!$checkout_id): ?>
                <form method="POST">
                    ...
                    <button type="submit" class="btn-primary">
                        <i class="fa-solid fa-coins"></i> <i class="fa-solid fa-mobile-screen"></i> Pay KSh <?= number_format($listing['price'], 2) ?>
                    </button>
                </form>
...
var interval = setInterval(function() {
    ...
    fetch('/ums/mpesa/status.php?checkout_id=' + checkoutId)
        ...
        .then(function(data) {
            if (data.status === 'success') {
                ...
                document.getElementById('payResult').innerHTML =
                    ...
                    '<p style="font-size:2rem"><i class="fa-solid fa-circle-check"></i></p>' +
                ...
                document.getElementById('payResult').innerHTML =
                    ...
                    '<p style="font-size:2rem"><i class="fa-solid fa-circle-xmark"></i></p>' +
                    '<h3>Payment Failed</h3>' +
                    '<p>' + (data.result_desc || 'Payment was not completed.') + '</p>' +
                    '<a href="/ums/pay.php?listing_id=' + listingId + '" ' +
                    'class="btn-primary" ' +
                    'style="display:inline-block;width:auto;padding:10px 24px;margin-top:12px;">' +
                    'Try Again</a></div>';
                ...
                var secs = (maxAttempts - attempts) * 5;
                document.getElementById('payTimer').textContent =
